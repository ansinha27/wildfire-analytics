...
class TestLinearAssimilator:
    ...
    def test_assimilation_reduces_mse(self, tmp_path):
        ...
        background = truth + np.random.default_rng(1).normal(
            0, 0.1, shape
        ).astype(np.float32)

        # observations - closer to truth than background
        obs = truth + np.random.default_rng(2).normal(
            0, 0.05, shape
        ).astype(np.float32)
        ...
        assert results["mse_latent"] < mse_bg_latent, \
    ...
    def test_beta_affects_gain(self):
        ...
        low_beta = LinearAssimilator(
            compressor=compressor, mean=mean, beta=0.1
        )
        high_beta = LinearAssimilator(
            compressor=compressor, mean=mean, beta=10.0
        )
        ...
        assert np.linalg.norm(K_low) > np.linalg.norm(K_high), \
